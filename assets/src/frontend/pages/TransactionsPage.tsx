  ...
  return (
    <div className="space-y-4">
      <Card className="border-none bg-linear-to-r from-white via-slate-50 to-emerald-50 shadow-sm ring-1 ring-slate-200">
          ...
          <div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1">
                ...
                <CardDescription>
                  Analysez les opérations par compte et période avec des filtres
                  chargés depuis WordPress AJAX.
                </CardDescription>
              </div>
            </div>
        ...
        <CardContent className="grid gap-3 px-4 pb-4 sm:px-5 xl:grid-cols-[minmax(14rem,1.3fr)_12rem_12rem_auto] xl:items-end">
      ...
      <Card className="min-w-0 shadow-sm">
        <CardHeader className="gap-3 border-b border-slate-200/70 px-4 pb-4 sm:px-5">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <CardTitle>Journal des opérations</CardTitle>
              <CardDescription>
                {accountId !== "all"
                  ? `Compte sélectionné: ${accountId}`
                  : "Tous les comptes affichés"}
              </CardDescription>
            </div>
            <div className="flex flex-wrap gap-2">
              {(dateStart || dateEnd) && (
        ...
        <CardContent className="min-w-0 px-0 pb-0 pt-0 sm:px-0">
          <Separator className="mb-6" />

          {loading ? (
            <div className="flex min-h-[24rem] items-center justify-center rounded-[1.75rem] border border-dashed border-slate-200 bg-slate-50 text-sm text-slate-500">
                  ...
                  {transactions.map((transaction) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell className="max-w-[16rem] font-medium text-slate-900">
                          {transaction.name ?? "—"}
                        </TableCell>
                        <TableCell className="max-w-[22rem] truncate text-sm text-slate-600">
                          {transaction.memo ?? "—"}
                        </TableCell>
